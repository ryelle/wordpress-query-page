...
function requests() {
	...
	switch (action.type) {
		...
		case PAGE_REQUEST_FAILURE:
			return Object.assign({}, state[action.pagePath], _defineProperty({}, action.pagePath, PAGE_REQUEST === action.type));
		default:
			return state;
	}
}
...
function requestPage(path) {
	return function (dispatch) {
		...
		return site.pages().path(path).embed().then(function (data) {
			var page = data[0];
			dispatch({
